...
jsonData = spark.read.schema(schema).json(r"D:\ETL\Practice\HomeWork-ETL\Spark_SQL\data\2015-03-01-17.json")

# jsonData.select('id').show()
# jsonData.select(col('id')).show()

# jsonData.select(col('id'), col('id') > 2615567688).show()

#Select with alias = SQL (as)

# jsonData.select(col('id').alias('user_id'), col('type').alias('user_type'), col('actor.id')).show()
...
# jsonData.select(
#     col('id'),
#     (col('id') < 2615567688).alias('smaller_than_2615567688')
# ).show()
